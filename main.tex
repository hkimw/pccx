...
%  programming reference manuals (heavy TikZ architectural diagrams,
...
% ---------- Bit-level encoding diagrams (reference-manual style) ----------
\usepackage{bytefield}

% ---------- Boxed instruction reference blocks (classical CPU reference-manual style) ----
...
\hypersetup{
    ...
    pdfkeywords= {pccx, NPU, ISA, KV260, DSP48E2, systolic array, GEMM, GEMV, LLM, Transformer, software developer manual}
}
...
% ---------- Classical reference-manual Instruction Reference environment ----
...
% ---------- Reference-manual-style "sub-block" inside InstructionRef ----------
...
% ---------- TikZ common styles ----------
% Classical reference-manual minimalism: thin black borders, light fills, black arrows.
\tikzset{
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=south west, text=white, font=\sffamily\small]
    ...
    {Document Repository: \texttt{https://github.com/hkimw/pccx}};
...
is the per-instruction reference written in the classical CPU
reference-manual format (Opcode table, Description, Operation
pseudocode, Flags Affected, Exceptions); Volume 3 is the
\textbf{system programming
...
The pccx web portal at \url{https://hkimw.github.io/pccx/}
...
repository
\href{https://github.com/hkimw/pccx-FPGA-NPU-LLM-kv260}{\texttt{hkimw/pccx-FPGA-NPU-LLM-kv260}}
under
...
\begin{tikzpicture}[
  ...
  \node[blocknode, right=0.35cm of sfu, minimum width=2.3cm, fill=accentorange!10] (dma) {DMA\\\scriptsize HP$\times$4 fetch};

  % Row 3 — Activation L1 buffers (act. path: ACP→L2→L1→Compute)
  %          + Buffer Queue (weight path: HP→BufQ→PipeReg→Compute, bypasses L1/L2)
  \node[ram, below=1.0cm of gemm, minimum width=2.3cm] (l1g) {L1 (Act.)\\\scriptsize BRAM};
  \node[ram, below=1.0cm of gemv, minimum width=2.3cm] (l1v) {L1 (Act.)\\\scriptsize BRAM};
  \node[ram, below=1.0cm of sfu,  minimum width=2.3cm] (l1s) {L1\\\scriptsize SFU};
  \node[ram, below=1.0cm of dma,  minimum width=2.3cm, fill=accentorange!15]
    (wfif) {Buffer Queue\\\scriptsize HP ingress};

  % Pipeline registers (between Buffer Queue and GEMM/GEMV, weight path)
  \node[stage, below=0.4cm of wfif, minimum width=2.3cm, minimum height=0.65cm, fill=black!5]
    (pr) {Pipe.~Reg.};

  % L2 — wide bar; receives activations from L1 and from ACP (coherent)
  ...
  % Activation path (ACP → L2 → L1 → Compute)
  \draw[bus] (gemm) -- (l1g);
  \draw[bus] (gemv) -- (l1v);
  \draw[bus] (sfu)  -- (l1s);
  \draw[bus] (l1g)  -- (l2);
  \draw[bus] (l1v)  -- (l2);
  \draw[bus] (l1s)  -- (l2);
  % ACP feeds L2 (coherent, from left)
  \draw[arr] ($(l2.west)+(-0.6cm,0)$) -- node[above, font=\sffamily\tiny, fill=white, inner sep=1pt]{ACP} (l2.west);

  % Weight path: HP → DMA → Buffer Queue → Pipe.Reg → GEMM/GEMV (bypasses L1/L2)
  \draw[bus] (dma) -- (wfif);
  \draw[bus] (wfif) -- (pr);
  % Dashed arrow from Pipe.Reg exiting left, labelled — goes to GEMM and GEMV directly
  \draw[ctrl] (pr.west) -- ++(-0.6cm,0)
      node[anchor=east, font=\sffamily\tiny, align=right, text width=2.0cm]
          {$\to$ GEMM/GEMV\\(bypasses L1/L2)};

  \draw[arr] (cc.east) -| ++(0.6cm,1.6cm) |- (disp.east);
\end{tikzpicture}
\caption{NPU top-level block diagram. \textbf{Activation path} (solid bus):
ACP $\to$ L2 $\to$ L1(Act.)~$\to$ compute.  \textbf{Weight path} (labelled
dashes): AXI-HP $\to$ Buffer Queue $\to$ Pipe.Reg $\to$ GEMM/GEMV direct,
bypassing L1 and L2.  SFU uses L1/L2 for CVO operands.}
\label{fig:npu-top}
\end{figure}
...
\begin{tikzpicture}[
  ...
  \node[blocknode, right=1.8cm of wr, minimum width=3.6cm, minimum height=0.9cm, fill=black!4]
    (fifo) {XPM Async FIFO\\\scriptsize 4096$\times$128\,b};
  \node[blocknode, right=1.8cm of fifo, minimum width=2.6cm, minimum height=0.9cm, fill=accentgreen!10]
    (rd) {Read\\\scriptsize 400\,MHz};

  % Arrows — labels with white fill to prevent overlap
  \draw[arrfat] (wr) -- node[above, font=\sffamily\tiny, fill=white, inner sep=1.5pt]{data + wr\_en} (fifo.west);
  \draw[arrfat] (fifo.east) -- node[above, font=\sffamily\tiny, fill=white, inner sep=1.5pt]{data + rd\_en} (rd);
...
\begin{tikzpicture}[
  ...
  \node[rectangle, draw=black, thin, fill=accentorange!15, minimum width=9.0cm, minimum height=0.9cm, below=of ddr] (hp) {AXI-HP $\times$4 --- 16\,GB/s burst \textcolor{graytext}{\scriptsize (weight/act. DMA, non-coherent)}};
  \node[rectangle, draw=black, thin, fill=accentorange!8,  minimum width=8.0cm, minimum height=0.9cm, below=of hp]  (acp) {ACP --- low-latency coherent \textcolor{graytext}{\scriptsize (L2-coherent host$\leftrightarrow$device)}};
  ...
  \node[font=\sffamily\scriptsize\color{graytext}, right=0.2cm of hp]  {$\sim$40 cyc};
...
\end{tikzpicture}
\caption{Memory hierarchy pyramid. AXI-HP~$\times$4 carries non-coherent
weight/activation DMA (direct to Buffer Queue, bypassing L2); ACP provides
coherent host$\leftrightarrow$L2 transfers. The unified 1.75\,MB L2 dominates
the on-chip budget.}
\label{fig:mem-hier}
\end{figure}
...
\begin{tikzpicture}[
  ...
  % Legend — placed to the right of the grid (grid right edge ≈ x = 5.13cm)
  \draw[fill=accentorange!30, draw=black, thin] (5.5,-0.2) rectangle ++(0.45,0.45);
  \node[anchor=west, font=\sffamily\small] at (6.05,-0.0)
    {Weight FIFO};
  \node[anchor=west, font=\sffamily\scriptsize\color{graytext}] at (6.05,-0.35)
    {8 URAMs, 2 per port $\times$ 4};
  \draw[fill=accentpurple!20, draw=black, thin] (5.5,-1.1) rectangle ++(0.45,0.45);
  \node[anchor=west, font=\sffamily\small] at (6.05,-0.9)
    {Unified L2};
  \node[anchor=west, font=\sffamily\scriptsize\color{graytext}] at (6.05,-1.25)
    {56 URAMs, 28 pairs};
\end{tikzpicture}
...
pccx uses a \textbf{W4A8} quantisation scheme: weights are stored as
INT4 and activations (feature maps, FMAP) are kept as INT8 throughout
the datapath.  A single DSP48E2 executes the 9-bit~$\times$~4-bit inner
multiplication natively.  The accumulated integer result is scaled by the
joint BF16 factor $S_{\mathrm{fmap}} \cdot S_{\mathrm{weight}}$
\textit{after} accumulation, in the Post-process stage.

Feature maps remain INT8 for all linear layers (Q/K/V projection, FFN).
Promotion to BF16 is performed \textit{only} before entering the Special
Function Unit (SFU) for operations that require floating-point precision:
RMSNorm, RoPE ($\sin$/$\cos$), softmax, and GELU.  This keeps BF16
traffic confined to the SFU lane and off the main datapath.
...
\begin{tikzpicture}[
  ...
  % Inputs
  \node[blocknode, fill=accentorange!10] (act) {Activation\\\scriptsize INT8 (FMAP)};
  ...
  \node[blocknode, right=1.1cm of act] (emax)  {Scale align\\\scriptsize $S_f$ factor};
  \node[blocknode, right=1.1cm of w]   (signx) {Sign-extend\\\scriptsize INT4$\to$INT9};

  % DSP48E2 — centred between emax and signx, with gap (2.4 cm) to balance arrow lengths
  \node[blocknode, fill=accentred!8,
        right=2.4cm of $(emax)!0.5!(signx)$,
  ...
  \draw[arr] (post)  -- (o);
\end{tikzpicture}
\caption{DSP48E2 datapath (W4A8). INT8 activations are sign-extended to
INT9; INT4 weights are sign-extended to INT4. The 9$\times$4\,b product
accumulates in a 48-bit DSP48E2 register; the joint BF16 scale factor
$S_f \cdot S_w$ is applied once in the Post-process stage.}
\label{fig:dsp-datapath}
\end{figure}

\section{Systolic Dataflow — Weight Stationary (GEMM)}

In the Weight Stationary schedule, a tile of INT4 weights is preloaded
once into the 32$\times$32 PE array and held fixed for the duration of
the tile computation.  Activations (INT8, one row per cycle) enter from
the left and propagate right; partial sums (48-bit) enter from the top
and propagate downward.  Each PE performs one MAC per cycle with no data
movement on the weight side, maximising weight reuse and minimising the
HP bandwidth required per GFLOP.

After the tile completes, the accumulator values exit at the bottom edge
of the array, pass through the Post-process stage (BF16 scale multiply),
and are written back to L2.  The next weight tile is then preloaded and
the loop repeats.  See Figure~\ref{fig:systolic} for the dataflow diagram.
...
\begin{tikzpicture}[
  ...
  % Row/col labels: physical indices 0,1,2,3,4,5 map to logical 0,1,2,...,30,31
  % Index 3 is the ellipsis slot; 0,1,2 → 0,1,2; 4 → 30; 5 → 31

  % ---- 6×6 PE array (represents 32×32; row/col 3 = ellipsis slot) ----
  \foreach \i in {0,...,5}
    \foreach \j in {0,...,5} {
      \pgfmathtruncatemacro{\isDots}{(\i==3 || \j==3) ? 1 : 0}
      \ifnum\isDots=1
        \node[pe, fill=white, draw=white] (pe\i\j) at (\j*\step, -\i*\step) {$\cdots$};
      \else
        \pgfmathtruncatemacro{\ri}{(\i<3) ? \i : (\i==4 ? 30 : 31)}
        \pgfmathtruncatemacro{\rj}{(\j<3) ? \j : (\j==4 ? 30 : 31)}
        \node[pe] (pe\i\j) at (\j*\step, -\i*\step) {$W_{\ri,\rj}$};
      \fi
    }
  ...
  \foreach \i in {0,1,2,4,5} {
    ...
    \node[anchor=east, font=\scriptsize] at (-1.05, -\i*\step) {$a_{\ri}$};
  }
  % Ellipsis for activation row 3
  \node[anchor=east, font=\scriptsize] at (-1.05, {-3*\step}) {$\vdots$};

  % ---- Partial-sum inputs from top ----
  \foreach \j in {0,1,2,4,5} {
    \pgfmathtruncatemacro{\rj}{(\j<3) ? \j : (\j==4 ? 30 : 31)}
    \draw[arr] ({\j*\step}, {0.70}) -- ({\j*\step}, {\half+0.02});
    \node[anchor=south, font=\scriptsize] at (\j*\step, 0.72) {$\psi_{\rj}^{\mathrm{in}}$};
  }
  \node[anchor=south, font=\scriptsize] at ({3*\step}, 0.72) {$\cdots$};

  % ---- Partial-sum outputs from bottom ----
  \foreach \j in {0,1,2,4,5} {
    \pgfmathtruncatemacro{\rj}{(\j<3) ? \j : (\j==4 ? 30 : 31)}
    \draw[arr] ({\j*\step}, {-5*\step-\half-0.02}) -- ({\j*\step}, {-5*\step-0.72});
    \node[anchor=north, font=\scriptsize] at (\j*\step, {-5*\step-0.74}) {$\psi_{\rj}^{\mathrm{out}}$};
  }
  \node[anchor=north, font=\scriptsize] at ({3*\step}, {-5*\step-0.74}) {$\cdots$};
  ...
  \node[anchor=north west, font=\sffamily\small, align=left]
    at (0, {-5*\step-1.0}) {Weights $W_{i,j}$ preloaded per tile (Weight Stationary).\\
                             Activations $a_i$ stream left-to-right ($i=0,1,\ldots,31$);\\
                             partial sums $\psi_j$ top-to-bottom ($j=0,1,\ldots,31$).};
\end{tikzpicture}
\caption{Systolic array dataflow (32$\times$32; middle row/column
compressed for clarity). Weights $W_{i,j}$ are preloaded once per tile;
activations $a_i$ stream left-to-right; partial sums $\psi_j$ propagate
top-to-bottom every cycle.}
\label{fig:systolic}
\end{figure}
...
\begin{tikzpicture}[
  ...
  % Core → reduce fan-in (trunk symmetrical to weight bus fan-out)
  \coordinate (rjct) at ($(red.west)-(0.55cm,0)$);
  \draw[trunk] (rjct |- c0) -- (rjct |- c3);  % vertical trunk spanning all cores
  \foreach \c in {c0,c1,c2,c3} \draw[-] (\c.east) -- (rjct |- \c);
  \draw[arr] (rjct) -- (red.west);
...
\begin{tikzpicture}[
  ...
  % Branches → Dispatcher (trunk fan-in, symmetric to the fan-out above)
  \coordinate (jct2) at ($(disp.west)-(0.65cm,0)$);
  \draw[trunk] ($(jct2)+(0, 2.16cm)$) -- ($(jct2)+(0,-2.16cm)$);
  \foreach \b in {b0,b1,b2,b3,b4} \draw[-] (\b.east) -- (jct2 |- \b);
  \draw[arr] (jct2) -- (disp.west);
...
\begin{bytefield}[bitwidth=0.68em, bitheight=2.4em,
  ...
  \bitbox{3}{\tiny rsv\\[-0.4em]\tiny[2:0]}
\end{bytefield}
...
\begin{bytefield}[bitwidth=0.68em, bitheight=2.4em,
  ...
  \bitbox{4}{opcode\\\scriptsize[63:60]} &
  \bitbox{2}{\tiny dc\\[-0.3em]\tiny[59:58]} &
  ...
  \bitbox{4}{\tiny rsv\\[-0.3em]\tiny[3:0]}
\end{bytefield}
...
\begin{bytefield}[bitwidth=0.68em, bitheight=2.1em, boxformatting={\centering\sffamily\scriptsize}]{64}
  \bitheader[endianness=big,lsb=0]{0,1,7,24,41,58,59,60,63} \\
  \bitbox{4}{opcode} &
  \bitbox{1}{\tiny fd} & \bitbox{1}{\tiny td} &
  \bitbox{17}{dest\_addr\\\scriptsize[57:41]} &
  \bitbox{17}{src\_addr\\\scriptsize[40:24]} &
  \bitbox{17}{aux\_addr\\\scriptsize[23:7]} &
  \bitbox{6}{shape\_ptr\\\scriptsize[6:1]} &
...
\begin{tikzpicture}[
  ...
  \draw[arr]        (l2) to [bend right=25] node[above, pos=0.5, fill=white, inner sep=1pt]{\texttt{8'h10}} (host);
  ...
  \draw[arr, dashed] (sfu) to [bend right=25] node[above, pos=0.5, fill=white, inner sep=1pt]{\texttt{8'h41}} (l2);
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  \node[left=0.1cm of g1, font=\sffamily\bfseries\color{oceanblue}] {GEMM};
  \foreach \a/\b in {g1/g2, g2/g3, g3/g4, g4/g5, g5/g6} \draw[arr] (\a)--(\b);

  % GEMV path
  \node[dfstep, below=1.0cm of g1] (v1) {Const\,Cache\\lookup};
  ...
  \node[left=0.1cm of v1, font=\sffamily\bfseries\color{oceanblue}] {GEMV};
  \foreach \a/\b in {v1/v2, v2/v3, v3/v4, v4/v5, v5/v6} \draw[arr] (\a)--(\b);

  % MEMCPY path
  \node[dfstep, below=1.0cm of v1] (m1) {Resolve\\route};
  ...
  \node[left=0.1cm of m1, font=\sffamily\bfseries\color{oceanblue}] {MEMCPY};
  \foreach \a/\b in {m1/m2, m2/m3, m3/m4, m4/m5, m5/m6} \draw[arr] (\a)--(\b);

  % CVO path
  \node[dfstep, below=1.0cm of m1] (c1) {Func\\decode};
  ...
  \node[left=0.1cm of c1, font=\sffamily\bfseries\color{oceanblue}] {CVO};
...
\begin{tikzpicture}[
  ...
  \node[scheduler, below=0.55cm of rh, minimum width=9cm]
  ...
  \draw[arr] (ah) -- (rh);
  \draw[arr] (rh) -- (ex);
  \draw[arr] (ex.south) -- (fence.north -| ex);
...
\endfirsthead
v002 & 2026-04-21 &
Initial Software Developer's Manual release.  Classical
reference-manual-style per-instruction reference blocks, bit-level
Type A/B/C/D \texttt{bytefield} diagrams, print-quality TikZ architectural
...
\hline
v002-r2 & 2026-04-22 &
Reference-manual polish pass.  Added wire-level AXI-HP read-handshake
